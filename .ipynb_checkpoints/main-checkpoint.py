...
def classify_signal(file, reference_hyper, reference_hypo, threshold=0.8):
    ...
    if similarity_hyper >= threshold and similarity_hyper > similarity_hypo:
        ...
        print(f"{file} est plus similaire à 'hyper' avec un coefficient de {similarity_hyper:.2f} contre {similarity_hypo:.2f}")

    elif similarity_hypo >= threshold and similarity_hypo > similarity_hyper:
        hypo_signals.append(file)
        print(f"{file} est plus similaire à 'hypo' avec un coefficient de {similarity_hypo:.2f} contre {similarity_hyper:.2f}")

    else:
        other_signals.append(file)
        print(f"Pas similaire à 'hyper' ou 'hypo'")
